...
function mapBundleRulePublic(r) {
  const out = {
...
function filterBundleRulesForProduct(rows, productId) {
...
export async function listYaadroHomeSections(shopId) {
  ...
  const bxgyLabel = (buyQty, getQty, dealMode = 'same_sku') => {
    ...
    if (dealMode === 'cross_sku') {
      if (buy === 1 && get === 1) return 'Buy one product, get another free';
      return `Buy ${buy}, get ${get} free of a different product`;
    }
    if (buy === 1 && get === 1) return 'Buy 1 Get 1 Free — same product';
    return `Buy ${buy} Get ${get} free — same product`;
  };
        ...
        pushDeal({
          ...
          headline:
            bq === 1 && gq === 1
              ? 'Buy 1 Get 1 Free'
              : `Buy ${bq} Get ${gq} Free`,
          hint: 'Same product — add the paid qty, free units apply at checkout',
        });
        continue;
      }
            ...
            pushDeal({
              ...
              headline:
                defaultBuyQty === 1 && defaultGetQty === 1
                  ? 'Buy this → get that free'
                  : `Buy ${defaultBuyQty} → get ${defaultGetQty} free`,
              hint: `Buy ${defaultBuyQty} of the left item to unlock ${defaultGetQty} free of the right item`,
            });
          });
        } else {
          // Ambiguous many-to-many: still pair each buy with each get (cap) so customer sees every combo.
          const cap = 12;
          for (const b of buys) {
            for (const g of gets) {
