...
import { query, queryWithShop, withShopContext } from './db';

/** @type {boolean | null} */
let yaadroCatalog = null;
/** @type {boolean | null} */
let yaadroShopProducts = null;

/** True when both shop_products and global_categories exist (full Yaadro catalog). */
...
/** True when shop_products exists — prefer status-filtered Yaadro product SQL. */
export async function hasYaadroShopProducts() {
  if (yaadroShopProducts != null) return yaadroShopProducts;
  const { rows } = await query(
    `SELECT 1
       FROM information_schema.tables
      WHERE table_schema = 'public'
  ...
  yaadroShopProducts = rows.length > 0;
  return yaadroShopProducts;
}

export function resetYaadroCatalogCache() {
  yaadroCatalog = null;
  yaadroShopProducts = null;
}
...
function mediaUrl(storageKey, imageUrl) {
  ...
  const key = String(storageKey || url || '')
    .trim()
    .replace(/^\/+/, '');
  if (!key) return null;
...
function mapYaadroProduct(row) {
  ...
  return {
    ...
    soldByWeight: row.sold_by_weight === true,
    unit_size: row.unit_size != null ? String(row.unit_size) : null,
    unitSize: row.unit_size != null ? String(row.unit_size) : null,
    base_unit: row.base_unit != null ? String(row.base_unit) : null,
    unit: row.base_unit != null ? String(row.base_unit) : null,
  };
}

/** Shop overlay first, then global product gallery (same tables admin writes). */
const PRODUCT_IMAGE_JOINS = `
    LEFT JOIN LATERAL (
    ...
    LEFT JOIN media_assets spi_ma ON spi_ma.id = spi.media_asset_id
    LEFT JOIN LATERAL (
    ...
    LEFT JOIN media_assets gpi_ma ON gpi_ma.id = gpi.media_asset_id
`;
...
const PRODUCT_SELECT = `
  SELECT sp.id, sp.shop_id, sp.status, sp.availability,
         ...
         sp.sold_by_weight, sp.unit_size, sp.base_unit,
         ...
         COALESCE(sp.global_brand_id, gp.global_brand_id) AS brand_id,
         COALESCE(NULLIF(BTRIM(sp.image_url), ''), gp.image_url) AS image_url,
         COALESCE(spi_ma.storage_key, gpi_ma.storage_key) AS image_storage_key,
         c.name AS category_name,
         c.slug AS category_slug,
         NULLIF(BTRIM(b.name), '') AS brand_name
    ...
    LEFT JOIN global_brands b
      ON b.id = COALESCE(sp.global_brand_id, gp.global_brand_id)
 ...
 */
/**
 * Load name/image snapshots for buy/reward SKUs referenced by bundle rules
 * but not present on the current product page.
 */
async function loadYaadroProductSnapshots(shopId, productIds) {
  const ids = [...new Set((productIds || []).map(String).filter(Boolean))];
  if (!shopId || ids.length === 0) return new Map();
  try {
    const { rows } = await queryWithShop(
      shopId,
      `SELECT sp.id,
              COALESCE(sp.name, gp.name) AS name,
              COALESCE(NULLIF(BTRIM(sp.image_url), ''), gp.image_url) AS image_url
...
/**
 * Attach active BXGY `bundleRules` + promo unit pricing to list/PDP products
 * (same engine data home sections already use). Without this, add-to-cart cannot
 * synthesize free reward lines from product detail or category lists.
 */
async function attachYaadroPromoFields(shopId, products) {
  const list = Array.isArray(products) ? products.filter(Boolean) : [];
  if (!shopId || list.length === 0) return list;

  const pageIds = list.map((p) => String(p.id)).filter(Boolean);
  const promoCtx = await loadYaadroListingPromoContext(shopId, pageIds);

  const relatedIds = new Set(pageIds);
  for (const r of promoCtx.bundleRows || []) {
    if (r.shop_product_id) relatedIds.add(String(r.shop_product_id));
    if (r.buy_shop_product_id) relatedIds.add(String(r.buy_shop_product_id));
    if (r.reward_shop_product_id) relatedIds.add(String(r.reward_shop_product_id));
  }
  const missingIds = [...relatedIds].filter((id) => !pageIds.includes(id));
  const extraById = await loadYaadroProductSnapshots(shopId, missingIds);
  ...
  for (const p of list) {
    productById.set(String(p.id), {
      ...
      imageUrl: p.imageUrl || p.thumbnail?.url || null,
    });
  }
  for (const [id, snap] of extraById) {
    productById.set(id, snap);
  }

  return list.map((p) => {
    const pid = String(p.id);
    const listMinor = toMinor(p.actual_price_minor ?? p.price_minor_per_unit);
    const offerMinor = toMinor(p.offer_price_minor_per_unit);
    const baseline =
      offerMinor > 0 && offerMinor < listMinor ? offerMinor : offerMinor || listMinor;
    const promoPriceMinor = pickWinningPromoPrice(promoCtx.overlaysByProduct, pid);
    const finalMinor = promoPriceMinor != null ? promoPriceMinor : baseline;
    const hasDiscount = listMinor > 0 && finalMinor < listMinor;

    let bundleRules = filterBundleRulesForProduct(
      promoCtx.bundleRows,
      pid,
      productById
    );
    bundleRules = bundleRules.map((rule) => {
      const next = { ...rule };
      const buyId = String(rule.buy_shop_product_id ?? rule.shop_product_id ?? '');
      const getId = String(rule.reward_shop_product_id ?? '');
      const buyP = buyId ? productById.get(buyId) : null;
      const getP = getId ? productById.get(getId) : null;
      if (buyP?.name) next.buy_product_name = buyP.name;
      if (getP?.name) next.reward_product_name = getP.name;
      if (getP?.imageUrl) next.reward_product_image = getP.imageUrl;
      return next;
    });
    ...
    return {
      ...
      price_minor_per_unit: listMinor,
      final_price_minor: finalMinor,
      total_discount_minor: hasDiscount ? Math.max(0, listMinor - finalMinor) : 0,
      promo_price_minor_per_unit: promoPriceMinor,
      offer_price_minor_per_unit: hasDiscount ? finalMinor : offerMinor || null,
      bundleRules,
...
export async function listYaadroProducts(opts = {}) {
  ...
  if (opts.categoryId) {
    if (opts.includeDescendants) {
      // Dairy (etc.) includes children like Ghee via recursive category walk.
      clauses.push(`COALESCE(sp.global_category_id, gp.global_category_id) IN (
        WITH RECURSIVE cat_tree AS (
          SELECT id FROM global_categories WHERE id = $${i}::uuid
          UNION ALL
          SELECT c.id
  ...
  if (opts.brandId) {
    clauses.push(`COALESCE(sp.global_brand_id, gp.global_brand_id) = $${i++}::uuid`);
  ...
  const sql = `
    ${PRODUCT_SELECT}
     ...
     LIMIT $${i++} OFFSET $${i}
  `;
  // FORCE RLS on shop_products requires app.current_shop_id (set via queryWithShop).
  const { rows } = await queryWithShop(shopId, sql, [...params, limit, offset]);
  const products = await attachYaadroPromoFields(
    shopId,
    rows.map(mapYaadroProduct)
  );
  return {
    products,
    nextCursor: null,
...
export async function getYaadroProduct(idOrSlug, shopId) {
  ...
  const [enriched] = await attachYaadroPromoFields(shopId, [
    mapYaadroProduct(rows[0]),
  ]);
...
export async function listYaadroCategories(opts = {}) {
  ...
  const params = [shopId];
  // Show shared/global categories + any category owned by this shop (admin "Shop" tag).
  // Product count is irrelevant — empty categories still appear.
  const clauses = [
    `(c.is_active IS NULL OR c.is_active = true)`,
    `(
       c.owner_shop_id = $1::uuid
       OR lower(coalesce(nullif(btrim(c.scope::text), ''), 'shared'))
            IN ('shared', 'global', 'public')
     )`,
    `NOT EXISTS (
  ...
  const { rows } = await queryWithShop(
    ...
    `SELECT c.id, c.parent_id, c.name, c.slug, c.sort_order, c.is_active,
            COALESCE(sci_ma.storage_key, gci_ma.storage_key) AS image_storage_key
       FROM global_categories c
       LEFT JOIN LATERAL (
         SELECT sci.media_asset_id
           FROM shop_category_images sci
          WHERE sci.global_category_id = c.id
            AND sci.shop_id = $1::uuid
          ORDER BY sci.sort_order ASC NULLS LAST
          LIMIT 1
       ) sci ON true
       LEFT JOIN media_assets sci_ma ON sci_ma.id = sci.media_asset_id
       LEFT JOIN LATERAL (
         SELECT gci.media_asset_id
           FROM global_category_images gci
          WHERE gci.global_category_id = c.id
          ORDER BY gci.sort_order ASC NULLS LAST
          LIMIT 1
       ) gci ON true
       LEFT JOIN media_assets gci_ma ON gci_ma.id = gci.media_asset_id
      WHERE ${clauses.join(' AND ')}
  ...
  return {
    categories: rows.map((row) => {
      ...
      return {
        ...
        image: imageUrl
          ? { url: imageUrl, storageKey: row.image_storage_key || undefined }
          : null,
        imageUrl,
        image_url: imageUrl,
...
export async function listYaadroCoupons(opts = {}) {
  ...
  const settingsRes = await queryWithShop(
    shopId,
...
/** Winning promo overlay per product (priority ASC, then newest). */
function pickWinningPromoPrice(overlaysByProduct, productId) {
  const list = overlaysByProduct.get(String(productId));
  ...
  const price = toMinor(sorted[0]?.promo_price_minor_per_unit);
  return price > 0 ? price : null;
}

function mapBundleRulePublic(r, productById = null) {
  const out = {
  ...
  if (productById && typeof productById.get === 'function') {
    const buyP = r.buy_shop_product_id
      ? productById.get(String(r.buy_shop_product_id))
      : r.shop_product_id
        ? productById.get(String(r.shop_product_id))
        : null;
    const getP = r.reward_shop_product_id
      ? productById.get(String(r.reward_shop_product_id))
      : null;
    if (buyP?.name) out.buy_product_name = buyP.name;
    if (getP?.name) out.reward_product_name = getP.name;
    if (getP?.imageUrl) out.reward_product_image = getP.imageUrl;
  }
  return out;
}

function filterBundleRulesForProduct(rows, productId, productById = null) {
  ...
  return rows
    ...
    .map((r) => mapBundleRulePublic(r, productById));
}

async function loadYaadroListingPromoContext(shopId, productIds) {
  const ids = [...new Set(productIds.map(String).filter(Boolean))];
  if (!shopId || ids.length === 0) {
    return { overlaysByProduct: new Map(), bundleRows: [], promotionsPaused: false };
  }

  return withShopContext(shopId, async (q) => {
    ...
    try {
      ...
      promotionsPaused = Boolean(settings.rows[0]?.promotions_paused);
    } catch {
      /* table may be absent */
    }

    if (promotionsPaused) {
      return { overlaysByProduct: new Map(), bundleRows: [], promotionsPaused: true };
    }
    ...
    try {
      ...
      for (const row of rows) {
        const pid = String(row.shop_product_id);
        if (!overlaysByProduct.has(pid)) overlaysByProduct.set(pid, []);
        overlaysByProduct.get(pid).push(row);
      }
    } catch {
      /* promotions tables may be absent */
    }
    ...
    return { overlaysByProduct, bundleRows, promotionsPaused: false };
  });
}
...
export async function listYaadroHomeSections(shopId) {
  ...
  // Promo context opens its own tenant transaction.
  const promoCtx = await loadYaadroListingPromoContext(shopId, uniqueIds);
  ...
  if (uniqueIds.length > 0) {
    ...
    productById = new Map(
      prod.rows.map((r) => {
        const listMinor = toMinor(r.price_minor_per_unit);
        const offerMinor = toMinor(r.offer_price_minor_per_unit);
        const baseline =
          offerMinor > 0 && offerMinor < listMinor ? offerMinor : offerMinor || listMinor;
        const promoPriceMinor = pickWinningPromoPrice(
          promoCtx.overlaysByProduct,
          r.id
        );
        // Match checkout: promo replaces baseline when present.
        const finalMinor = promoPriceMinor != null ? promoPriceMinor : baseline;
        const hasDiscount = listMinor > 0 && finalMinor < listMinor;
        const bundleRules = filterBundleRulesForProduct(promoCtx.bundleRows, r.id);
        return [
          String(r.id),
          {
            id: r.id,
            name: r.name,
            slug: r.slug,
            imageUrl: mediaUrl(r.image_storage_key, r.image_url),
            ...
            total_discount_minor: hasDiscount ? Math.max(0, listMinor - finalMinor) : 0,
            totalDiscountMinor: hasDiscount ? Math.max(0, listMinor - finalMinor) : 0,
            bundleRules,
            // Clients that only read offer_*: expose payable unit when discounted.
            offer_price_minor_per_unit: hasDiscount ? finalMinor : offerMinor || null,
            offerPriceMinorPerUnit: hasDiscount ? finalMinor : offerMinor || null,
          },
        ];
      })
    );
    // Second pass: attach buy/get product names onto cross rules for cart/list copy.
    for (const product of productById.values()) {
      if (!Array.isArray(product.bundleRules)) continue;
      product.bundleRules = product.bundleRules.map((rule) => {
        const next = { ...rule };
        const buyId = String(rule.buy_shop_product_id ?? rule.shop_product_id ?? '');
        const getId = String(rule.reward_shop_product_id ?? '');
        const buyP = buyId ? productById.get(buyId) : null;
  ...
  const bxgyLabel = (buyQty, getQty, dealMode = 'same_sku') => {
  ...
  const bxgyDealMode = (buyIds, getIds) => {
    const buys = asIdList(buyIds);
    const gets = asIdList(getIds);
  ...
  /** Pair each BXGY rule into a customer-facing deal (both sides must be in stock). */
  const buildBxgyDeals = (buyIds, getIds, sectionBuyQty, sectionGetQty) => {
    ...
    // Fallback when rules missing: still show clear pairs from Damaka lists.
    if (deals.length === 0) {
      const buys = orderByIds(buyIds);
      const gets = orderByIds(getIds);
          ...
          pushDeal({
            ...
            headline:
              defaultBuyQty === 1 && defaultGetQty === 1
                ? 'Buy 1 Get 1 Free'
                : `Buy ${defaultBuyQty} Get ${defaultGetQty} Free`,
          });
        }
      } else if (buys.length && gets.length) {
        if (buys.length === gets.length) {
          buys.forEach((b, i) => {
            const g = gets[i];
            pushDeal({
  ...
  for (const row of rows) {
    ...
    if (type === 'buy_x_get_y') {
      if (promoCtx.promotionsPaused) continue;
      ...
      const buyProducts = orderByIds(buyIds);
      const getProducts = orderByIds(getIds);
      const deals = buildBxgyDeals(
        buyIds,
        getIds,
        Number.isInteger(buyQty) ? buyQty : null,
        Number.isInteger(getQty) ? getQty : null,
      );
      // Hide Offer Damaka when nothing sellable / no complete deal pairs.
    ...
    const products = orderByIds(row.product_ids);
    // Product / event shelves: only show when enabled window already passed AND products exist.
