...
import {
  getYaadroProduct,
  hasYaadroShopCatalog,
  hasYaadroShopProducts,
  listYaadroCategories,
  listYaadroProducts,
} from './storefrontYaadroCatalog';
...
export function mapProductRow(row, cols) {
  ...
  const statusCol = firstCol(cols, ['status']);
  ...
  return {
    ...
    status: statusCol && row[statusCol] != null ? String(row[statusCol]) : undefined,
...
function buildProductWhere(cols, { shopId, categoryId, search, availability }) {
  ...
  // Yaadro shop_products uses TEXT status (draft|active|archived). Pending activation
  // is draft/archived — never list those on the customer storefront.
  const statusCol = firstCol(cols, ['status']);
  if (statusCol) {
    clauses.push(`${quoteIdent(statusCol)} = 'active'`);
  } else {
 ...
 * @param {boolean} [opts.includeDescendants]
...
export async function listProductsFromDb(opts = {}) {
  // Prefer Yaadro shop_products SQL (always status=active) whenever that table exists.
  if (await hasYaadroShopProducts()) {
    try {
      return await listYaadroProducts(opts);
    } catch (err) {
      console.warn(
        '[storefront] Yaadro product list failed, using legacy active-only filter:',
        err?.message || err
      );
    }
  }
...
export async function getProductFromDb(idOrSlug, shopId) {
  if (await hasYaadroShopProducts()) {
    try {
      return await getYaadroProduct(idOrSlug, shopId);
    } catch (err) {
      console.warn(
        '[storefront] Yaadro product detail failed, using legacy active-only filter:',
        err?.message || err
      );
    }
  }
  ...
  const statusCol = firstCol(productCols, ['status']);
  if (statusCol) {
    clauses.push(`${quoteIdent(statusCol)} = 'active'`);
  } else {
    const activeCol = firstCol(productCols, ['is_active', 'isActive', 'active']);
    if (activeCol) {
      clauses.push(
        `(${quoteIdent(activeCol)} = TRUE OR ${quoteIdent(activeCol)}::text IN ('true','1','active'))`
      );
...
export async function listCategoriesFromDb(opts = {}) {
  if (await hasYaadroShopCatalog()) {
    return listYaadroCategories(opts);
  }
  // Categories-only Yaadro schema (no shop_products) still uses global_categories + images.
  if (opts.shopId && (await tableExists('global_categories'))) {
    try {
      return await listYaadroCategories(opts);
    } catch (err) {
      console.warn(
        '[storefront] listYaadroCategories failed, trying generic category table:',
        err?.message || err
      );
    }
  }
