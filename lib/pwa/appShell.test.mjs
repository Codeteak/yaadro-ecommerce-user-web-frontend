...
test('unlockAppScroll clears body overflow even if previous value was hidden', async () => {
  const { body, documentElement } = installDomStub();
  const mod = await import(`./appShell.js?t=${Date.now()}-c`);
  mod.__resetAppScrollLockForTests();

  // Simulate Radix (or another locker) already having set body overflow before we lock.
...
test('syncAppScrollLockFromBody does not invent a lock from stale body overflow', async () => {
  const { body, documentElement } = installDomStub();
  const mod = await import(`./appShell.js?t=${Date.now()}-d`);
  mod.__resetAppScrollLockForTests();

  body.style.overflow = 'hidden';
  mod.syncAppScrollLockFromBody();
