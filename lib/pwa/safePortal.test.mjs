...
test('ensurePortalRoot does not re-append a body-level host', async () => {
  const { body } = installDomStub();
  const modPath = new URL('./safePortal.js', import.meta.url).href;
  const { ensurePortalRoot, PORTAL_ROOT_ID } = await import(`${modPath}?t=${Date.now()}-3`);

  const existing = document.createElement('div');
  existing.id = PORTAL_ROOT_ID;
  body.appendChild(existing);
  let appendCount = 0;
  const orig = body.appendChild;
  body.appendChild = function tracked(el) {
    appendCount += 1;
