...
export function unlockAppScroll() {
  ...
  // Always clear body overflow when fully unlocked.
  // Restoring a previous inline `hidden` (often left by Radix Dialog / RemoveScroll
  // racing with this helper) permanently stuck page scroll + `app-scroll-locked`.
...
/**
 * Keep `app-scroll-locked` in sync with the refcount only.
 * Never invent a lock from a stale `body.style.overflow` — that re-froze
 * `#app-scroll` after Radix/Dialog races cleared the refcount.
 */
export function syncAppScrollLockFromBody() {
  if (typeof document === 'undefined') return;
  const locked = appScrollLockCount > 0;
