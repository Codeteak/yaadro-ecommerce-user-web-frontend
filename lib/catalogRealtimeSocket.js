...
export function shouldUseSameOriginCatalogRealtime() {
  ...
  // Multi-tenant shop DNS: only force same-origin sockets when explicitly enabled.
  // Default stays cross-origin to NEXT_PUBLIC_CATALOG_REALTIME_URL / API host so
  // greens.yaadro.online can use customer.yaadro.online/socket.io (CORS allowlist)
  // when Next /socket.io rewrites are unavailable on the shop host.
  const sameOriginApi = process.env.NEXT_PUBLIC_USE_SAME_ORIGIN_API?.trim().toLowerCase();
  const explicitRealtimeUrl = process.env.NEXT_PUBLIC_CATALOG_REALTIME_URL?.trim();
  if ((sameOriginApi === 'true' || sameOriginApi === '1') && !explicitRealtimeUrl) {
    return true;
  }

  // Auto: shop and API share a host (e.g. customer.yaadro.online) → same-origin + rewrite.
  // Explicit REALTIME_URL to a different host wins via getCatalogRealtimeOrigin instead.
  const explicit = explicitRealtimeUrl;
  if (explicit) {
    try {
