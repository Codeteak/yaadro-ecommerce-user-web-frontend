...
export function shouldUseSameOriginCatalogRealtime() {
  ...
  // Multi-tenant shop DNS (greens.yaadro.online) already uses same-origin REST via
  // NEXT_PUBLIC_USE_SAME_ORIGIN_API — sockets must follow or CORS to customer.* fails.
  const sameOriginApi = process.env.NEXT_PUBLIC_USE_SAME_ORIGIN_API?.trim().toLowerCase();
  if (sameOriginApi === 'true' || sameOriginApi === '1') return true;
