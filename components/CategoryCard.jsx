...
export default function CategoryCard({ category, labelClassName = 'text-gray-900', compact = false }) {
  ...
  const tileClass = compact
    ? 'relative flex h-[72px] w-[72px] shrink-0 items-center justify-center overflow-hidden rounded-[20px] bg-gray-50 ring-1 ring-gray-100 transition-transform duration-200 group-hover:scale-[1.04] group-active:scale-[0.98]'
    : 'relative flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gray-50 shadow-[0_8px_22px_rgba(0,0,0,0.10)] ring-1 ring-gray-200 transition-all duration-200 group-hover:scale-[1.04] group-hover:shadow-[0_12px_28px_rgba(0,0,0,0.16)] sm:h-24 sm:w-24 md:h-28 md:w-28';
    ...
    >
      <div className={tileClass}>
        <Image
          ...
          sizes={compact ? '72px' : '(max-width: 640px) 80px, (max-width: 768px) 96px, 112px'}
          onError={() => {
      ...
      <h3
        className={`text-center leading-[1.15] max-w-[10ch] ${
          compact
            ? `text-[11px] font-bold uppercase tracking-wide ${labelClassName}`
            : `text-[11px] sm:text-xs md:text-sm font-semibold ${labelClassName}`
        }`}
        title={categoryName}
      >
        {compact ? (
